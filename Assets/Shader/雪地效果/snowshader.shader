Shader "tess"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 disp(float4 pos,float3 normal,float2 uv)
            {
                float d = SAMPLE_TEXTURE2D_LOD(_Splat,sampler_Splat,uv,1).r*_Displacement;
                pos.xyz = pos.xyz-normal*d;
                pos.xyz+=normal*_Displacement;
                return pos;
            }
			...
			{
				Varyings o;
				o.vertex = TransformObjectToHClip(disp(v.vertex,v.normal,v.uv));
                o.color = v.color;
				o.uv = v.uv;
				o.posWS = TransformObjectToWorld(v.vertex);
                o.normal = TransformObjectToWorldNormal(v.normal);
                o.tangentWS = float4(TransformObjectToWorldDir(v.tangentOS.xyz),1);
                o.bitangentWS = float4(cross(o.tangentWS.xyz,o.normal),1);
            ...
            half4 FragmentProgram(Varyings i) : SV_TARGET 
            {
                ...
                float3x3 TBN = {normalize(i.tangentWS.xyz), normalize(i.bitangentWS.xyz), normalize(i.normal.xyz)};
                float3 normalTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, i.uv), 1);
                normalTS.z = pow(1-pow(normalTS.x,2)-pow(normalTS.y,2),0.5);
                float3 normalWS = mul(normalTS,TBN);
                float nDotL = dot(normalize(i.normal),light.direction);
                real3 diffuse = nDotL*light.color;
                float3 color = 0;
                color = dot(light.direction,i.normal);

                
                half amount = SAMPLE_TEXTURE2D(_Splat,sampler_Splat,i.uv).r;
                float4 c = lerp(SAMPLE_TEXTURE2D(_SnowTex,sampler_SnowTex,i.uv)*_SnowColor,SAMPLE_TEXTURE2D(_GroundTex,sampler_GroundTex,i.uv)*_GroundColor,amount);
